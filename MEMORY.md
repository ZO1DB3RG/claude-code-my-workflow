...
## ZCode Migration (2026-07-30)

[LEARN:zcode-migration] **This fork was adapted Claude Code → ZCode (commit series aacf47c → ef5a135 on branch `adapt/zcode`).** Key facts for future work: (1) config lives in `.zcode/config.json` (NOT `settings.json`); there is NO `permissions` block — tool gating is per-skill `allowed-tools`, not a global allow/deny. (2) Hooks nest under `hooks.events.<Event>` with `hooks.enabled:true`; ZCode supports only 7 events (SessionStart/UserPromptSubmit/PreToolUse/PermissionRequest/PostToolUse/PostToolUseFailure/Stop) — Notification/PreCompact/SubagentStop are gone. (3) `effort:` is invalid on skills AND agents (removed everywhere); `model:` on skills is ignored, on agents it's parsed but all resolve to the single configured model (GLM-5.2). (4) Env var is `$ZCODE_PROJECT_DIR` (`$CLAUDE_PROJECT_DIR` still works as fallback). (5) Entry file is `AGENTS.md` (workspace) / `~/.zcode/AGENTS.md` (user).

[LEARN:zcode-migration] **The fan-out runtime works natively in ZCode.** Verified 2026-07-30 with a 2-reviewer parallel fan-out (both `subagent_type: Explore` + persona injection) on AGENTS.md: both ran concurrently, returned the shared FINDING/SCORECARD YAML schema, and independently converged on the same 2 MAJOR findings. Translation rule (see `.zcode/references/zcode-execution-overlay.md`): Claude `Task(subagent_type=X, context:fork)` → ZCode parallel `Agent` calls; fresh isolated context is the DEFAULT (no param); named specialists use **persona injection** (prepend the agent's `.zcode/agents/<name>.md` body into the prompt, `subagent_type: general-purpose` or `Explore` for read-only). Custom `subagent_type` registration needs a plugin (Plan A, not enabled).

[LEARN:zcode-migration] **Model-tier routing is persona-weighting now.** The original 70/20/10 Haiku/Sonnet/Opus cost split is inactive under the single GLM-5.2 model. The tier taxonomy was repurposed as a ROLE WEIGHT (high-judgment / review-critique / mechanical), expressed via persona strength in each agent's `.md`, not model selection. See `.zcode/rules/model-routing.md` (rewritten) + `.zcode/references/model-versions.md`. Diversity for the challenger≠auditor guard now comes from fresh-context forking, not different tiers.

[LEARN:zcode-migration] **Known doc drift left intentionally:** README.md + guide/workflow-guide.qmd still describe the upstream Claude Code template (Fable 5 / Opus tiers, "Claude Code" branding). `check-model-versions.sh` correctly flags these as stale superlatives. They are user-facing upstream docs, not migrated infrastructure — fix them only if you intend to re-publish the guide; otherwise the flags are expected.
