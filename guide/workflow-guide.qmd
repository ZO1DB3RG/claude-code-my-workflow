...
::: {.callout-note appearance="simple" title="ZCode-adapted fork"}
This is a **ZCode adaptation** of Pedro H. C. Sant'Anna's Claude Code workflow
(see `MEMORY.md` → "ZCode Migration"). The workflow, skills, agents, and quality
gates are unchanged in spirit; only the **runtime layer** was ported. Key
differences from the upstream Claude Code guide below:

- **Config:** `.zcode/config.json` (not `.claude/settings.json`); there is **no
  `permissions` block** — gating is per-skill `allowed-tools`. Hooks nest under
  `hooks.events.*` with `hooks.enabled: true`; only 7 hook events exist.
- **Entry file:** `AGENTS.md` (not `CLAUDE.md`). Directory is `.zcode/`.
- **Model:** a single configured model (GLM-5.2) — the Opus/Sonnet/Haiku tiers
  in this guide are **role weights expressed via persona**, not model pins
  (`.zcode/rules/model-routing.md`). The `effort:` axis does not exist.
- **Fan-out:** uses ZCode's `Agent` tool (the harness aliases `Task` ↔ `Agent`).
  Fresh/forked context is the **default** subagent behavior. Named specialists
  are invoked via **persona injection** — see the `/invoke-agent` skill and
  `.zcode/references/zcode-execution-overlay.md`. (The plugin `agents` field is
  recorded-but-not-executed in this ZCode build, so custom `subagent_type`
  values are not registered.)

Anywhere this guide says `.claude/`, `CLAUDE.md`, `settings.json`, `Task`,
`context: fork`, or names a Claude model tier, read it through that overlay.
:::
...
## What Makes ZCode / Claude Code Different

> This section describes the original Claude Code capabilities; in this ZCode
> fork the same capabilities hold (ZCode is the agent runtime here). Read names
> through the overlay at the top of this guide: `CLAUDE.md`→`AGENTS.md`,
> `Task`→`Agent`, etc.

ZCode/Claude Code runs on your computer with full access to your file system, terminal, and git. It works as a **CLI tool**, a **VS Code-style extension**, or through the **desktop app** --- same capabilities, same configuration, different interface. Here is what that enables:
...
| Persistent memory | AGENTS.md + MEMORY.md survive across sessions |
...
| Multi-agent workflows | 18 specialized agents (invoked via `/invoke-agent` + persona injection) for proofreading, layout, pedagogy, code review, fact-checking, AI-voice auditing, simulation + R-package review |
| Quality review | Advisory scoring inside `/commit` (halt + ask to override); targets: 80 commit, 90 PR, 95 excellence |
| CLI/headless mode | Run from scripts |
...
**Create your knowledge base** --- Open `.zcode/rules/knowledge-base-template.md` and fill in:
...
**Configure hooks** (ZCode) --- Review `.zcode/config.json`. Unlike Claude Code's `settings.json`, ZCode has **no `permissions` block** (tool gating is per-skill `allowed-tools`); hooks live under `hooks.events.*` with `hooks.enabled: true`. The template's hooks (git-guardrails, context-monitor, claim-reconcile, log-reminder) are already wired.
...
## AGENTS.md --- Your Project's Constitution

> In ZCode this file is `AGENTS.md` (Claude Code called it `CLAUDE.md`). The role is identical.

`AGENTS.md` is the single most important file. The agent reads it at the start of every session. But here is the critical insight: **the model reliably follows about 100--150 custom instructions.** Your system prompt already uses ~50, leaving ~100--150 for your project. AGENTS.md and always-on rules share this budget.

This means AGENTS.md should be a **slim constitution** --- short directives and pointers, not comprehensive documentation. Aim for ~120 lines:
...
Move everything else into `.zcode/rules/` files (with path-scoping so they only load when relevant).

```markdown
# AGENTS.md --- My Course Development
